...
_gweather_location_unref_no_check (GWeatherLocation *loc)
{
    ...
    if (loc->_zones) {
	for (i = 0; loc->_zones[i]; i++)
	    gweather_timezone_unref (loc->_zones[i]);
	g_free (loc->_zones);
    }
...
gweather_location_unref (GWeatherLocation *loc)
{
    ...
    g_return_if_fail (loc->ref_count > 1 || (!loc->db || !g_ptr_array_find (loc->db->locations_keepalive, loc, NULL)));
...
add_timezones (GWeatherLocation *loc, GPtrArray *zones)
{
    ...
    if (loc->_zones) {
	for (i = 0; loc->_zones[i]; i++)
	    g_ptr_array_add (zones, gweather_timezone_ref (loc->_zones[i]));
    } else if (loc->db && IDX_VALID(loc->db_idx)) {
